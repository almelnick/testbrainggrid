...
export async function POST(req: NextRequest) {
  ...
  const log: ExecutionLog = { id: executionId, clientId, clientName: client.name, startedAt: new Date().toISOString(), completedAt: null, status: 'running', adsProcessed: 0, totalSpend: 0 };
  Store.setLog(log);
  (async () => {
    try {
      const rawAds = demoMode ? generateMockAds() : await fetchFacebookAds(client.adAccountId, client.facebookToken);
      ...
      const aiResults = await analyzeAdsWithAI({ adData: adDataForLLM, benchmarkData: benchmarks, promptTemplate: prompt.content, apiKey, executionId, clientId });
      ...
      aggregated.forEach((ad) => {
        ...
        const result: AdResult = {
          id: `result-${executionId}-${ad.adId}`, executionId, clientId, adId: ad.adId, adName: ad.adName,
          campaignName: ad.campaignName, adsetName: ad.adsetName, objective: ad.objective,
          dateStart: ad.dateStart, dateStop: ad.dateStop,
          totalSpend: parseFloat(ad.spend.toFixed(2)), totalImpressions: ad.impressions, totalClicks: ad.clicks,
          totalAddToCarts: ad.addToCarts, totalCheckoutsInitiated: ad.checkoutsInitiated,
          totalPurchases: ad.purchases, totalPurchaseValue: parseFloat(ad.purchaseValue.toFixed(2)),
      ...
      Store.setLog({ ...log, completedAt: new Date().toISOString(), status: 'completed', adsProcessed: aggregated.length, totalSpend: parseFloat(totalSpend.toFixed(2)) });
    } catch (err: unknown) {
      Store.setLog({ ...log, completedAt: new Date().toISOString(), status: 'failed', error: err instanceof Error ? err.message : String(err) });
    }
  })();
...
function aggregateByAdName(ads: ReturnType<typeof parseAdInsights>) {
  ...
  for (const ad of ads) {
    ...
    if (!existing) { map.set(ad.adName, { ...ad }); }
    else { existing.spend += ad.spend; existing.impressions += ad.impressions; existing.clicks += ad.clicks; existing.addToCarts += ad.addToCarts; existing.checkoutsInitiated += ad.checkoutsInitiated; existing.purchases += ad.purchases; existing.purchaseValue += ad.purchaseValue; }
  }
