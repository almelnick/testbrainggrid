...
export default function Dashboard() {
  ...
  const load = async () => {
    ...
    setResults(r); setLogs(l); setClients(c);
  };
  ...
  const handleRun = async () => {
    ...
    if (!activeClient) return alert('No active client found. Add a client first.');
    setRunning(true);
    await fetch('/api/analysis/run', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ clientId: activeClient.id, demoMode: true }) });
  ...
  const categoryCounts = results.reduce<Record<string, number>>((acc, r) => { acc[r.performanceCategory] = (acc[r.performanceCategory] ?? 0) + 1; return acc; }, {});
        ...
        <div style={{ backgroundColor: '#161b27', border: '1px solid #1e293b', borderRadius: '12px', padding: '20px' }}>
          ...
          {logs.slice(0, 4).map((log) => (
            <div key={log.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 0', borderBottom: '1px solid #1e293b' }}>
              ...
              <span style={{ fontSize: '11px', padding: '2px 8px', borderRadius: '9999px', backgroundColor: log.status === 'completed' ? '#16a34a22' : '#2563eb22', color: log.status === 'completed' ? '#22c55e' : '#3b82f6' }}>
          ...
          <tbody>
            {results.slice(0, 6).map((r) => (
              <tr key={r.id} style={{ borderBottom: '1px solid #0f172a' }}>
                <td style={{ padding: '10px 12px', color: '#e2e8f0', maxWidth: '180px' }}><div style={{ whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{r.adName}</div></td>
