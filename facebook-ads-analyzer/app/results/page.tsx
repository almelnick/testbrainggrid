  ...
  return (
    <div className="p-8">
      ...
      <div style={{ display: 'flex', gap: '10px', marginBottom: '20px', flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        {CATEGORIES.map((cat) => { const cfg = CATEGORY_CONFIG[cat]; const count = results.filter((r) => r.performanceCategory === cat).length; return (
          <button key={cat} onClick={() => setFilter(cat)} style={{ padding: '6px 12px', borderRadius: '20px', border: `1px solid ${filter === cat ? cfg.color : '#334155'}`, backgroundColor: filter === cat ? cfg.color + '22' : 'transparent', color: filter === cat ? cfg.color : '#64748b', fontSize: '11px', cursor: 'pointer', fontWeight: 600, whiteSpace: 'nowrap' }}>{cfg.emoji} {cat} ({count})</button>
        ); })}
      </div>
      <div style={{ backgroundColor: '#161b27', border: '1px solid #1e293b', borderRadius: '12px', overflow: 'hidden' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
          <thead><tr style={{ borderBottom: '1px solid #1e293b', backgroundColor: '#0f1723' }}>
            {['Ad Creative','Spend','ROAS','Purchases','Revenue','CVR','CPP','Category',''].map((h) => (<th key={h} style={{ padding: '10px 14px', textAlign: 'left', fontSize: '11px', color: '#64748b', fontWeight: 600 }}>{h}</th>))}
          </tr></thead>
          <tbody>
            {filtered.map((r) => (<>
              ...
              {expanded === r.id && (
                <tr key={r.id + '-detail'}><td colSpan={9} style={{ padding: 0 }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px' }}>
                      ...
                      <div>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
                          {[['Impressions', formatNumber(r.totalImpressions)],['Clicks', formatNumber(r.totalClicks)],['CTR', `${(r.ctr).toFixed(2)}%`],['CPC', formatCurrency(r.cpc)],['CPM', formatCurrency(r.cpm)],['Add to Carts', formatNumber(r.totalAddToCarts)],['Checkouts', formatNumber(r.totalCheckoutsInitiated)],['Avg Order Value', formatCurrency(r.averageOrderValue)]].map(([label, value]) => (
        ...
        {filtered.length === 0 && <div style={{ textAlign: 'center', padding: '60px', color: '#475569' }}><div style={{ fontSize: '36px', marginBottom: '12px' }}>🎯</div><div>No results match your filters.</div></div>}
      </div>
    </div>
  );
}
