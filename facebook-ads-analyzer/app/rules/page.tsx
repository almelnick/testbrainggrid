...
export default function RulesPage() {
  ...
  const handleSave = async (category: string) => {
    ...
    setRules(await fetch('/api/rules').then((r) => r.json()));
  ...
  return (
    <div className="p-8">
      ...
      <div style={{ backgroundColor: '#1e3a5f22', border: '1px solid #1e40af55', borderRadius: '10px', padding: '14px 18px', marginBottom: '24px', fontSize: '13px', color: '#93c5fd' }}>
        <strong>ℹ How rules work:</strong> These thresholds inform the AI prompt. <code>Min Spend</code> = minimum spend to qualify for the category. <code>ROAS Multiplier</code> = required ROAS relative to account average (1.5 = 50% above average).
      </div>
            ...
            <div key={rule.category} style={{ backgroundColor: '#161b27', borderRadius: '12px', border: `1px solid ${isEditing ? cfg.color + '55' : '#1e293b'}`, overflow: 'hidden' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 20px', borderBottom: isEditing ? `1px solid ${cfg.color}33` : 'none' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '24px' }}>
                  ...
                  <button onClick={() => isEditing ? setEditing(null) : (setEditing(rule.category), setEditData({ ...rule }))} style={{ padding: '6px 14px', borderRadius: '6px', border: `1px solid ${isEditing ? '#334155' : cfg.color + '55'}`, backgroundColor: 'transparent', color: isEditing ? '#64748b' : cfg.color, fontSize: '12px', cursor: 'pointer', fontWeight: 600 }}>{isEditing ? 'Cancel' : 'Edit'}</button>
                </div>
              </div>
              {isEditing && (
                <div style={{ padding: '20px', display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '16px' }}>
                  {[['MINIMUM SPEND ($)','minSpend','number'],['ROAS VS BENCHMARK (×)','roasVsBenchmark','number'],['DESCRIPTION','description','text']].map(([label, key, type]) => (
                    <div key={key}>
                      <label style={{ fontSize: '11px', color: '#64748b', display: 'block', marginBottom: '6px', fontWeight: 600 }}>{label}</label>
                      <input type={type} step={type === 'number' ? '0.1' : undefined} value={(editData as Record<string, string | number>)[key] ?? ''} onChange={(e) => setEditData((d) => ({ ...d, [key]: type === 'number' ? Number(e.target.value) : e.target.value }))} style={{ width: '100%', padding: '8px 12px', borderRadius: '8px', border: '1px solid #334155', backgroundColor: '#0f1723', color: '#e2e8f0', fontSize: '13px' }} />
                    </div>
                  ))}
                  <div style={{ gridColumn: '1 / -1', display: 'flex', justifyContent: 'flex-end' }}>
                    <button onClick={() => handleSave(rule.category)} disabled={saving === rule.category} style={{ padding: '8px 24px', borderRadius: '8px', border: 'none', backgroundColor: cfg.color, color: '#fff', fontSize: '13px', fontWeight: 600, cursor: 'pointer' }}>{saving === rule.category ? 'Saving…' : 'Save Rule'}</button>
                  </div>
                </div>
              )}
            </div>
