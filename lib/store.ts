...
store.prompt = {
  ...
  content: `**Role:** You are a Senior Facebook Ads Media Buyer with extensive experience in e-commerce campaign optimization. Your primary goal is to maximize Return on Ad Spend (ROAS) and overall profitability by identifying high-impact ad creatives and categorizing their performance.

**Context:**
You are analyzing ad creative performance for an e-commerce client. You have been provided with two sets of data for the past 28 days:
1. **"Ad Creative Performance Data"**: A JSON array where each object details a unique ad creative and its performance metrics.
2. **"Overall Account Benchmark Data"**: A single object with aggregated KPIs for all ads in the account over the same period.

**Task:**
Evaluate each ad creative from the "Ad Creative Performance Data" against the "Overall Account Benchmark Data." For every ad creative, compare its calculated metrics (CTR, CPC, CPM, Cost Per Purchase, Conversion Rate, ROAS, Average Order Value) against the corresponding benchmark values.

**Performance Categorization Rules:**

1. **Spend Significance:**
   - Spend < $50 -> "INSUFFICIENT DATA/SPEND" at best
   - To qualify for "HELL YES" or "YES" -> spend MUST be >= $100

2. **Categories (evaluate AFTER spend rules):**
   - **"HELL YES"**: Spend >= $100 AND dramatically outperforms benchmarks (ROAS >= 1.5x benchmark, CVR significantly above, CPP well below). Clear 80/20 driver.
   - **"YES"**: Spend >= $100 AND clearly outperforms benchmarks on ROAS, CVR, and CPP. Reliably contributing.
   - **"MAYBE"**: Spend >= $50. Marginal outperformance or mixed results. Requires observation.
   - **"NOT REALLY"**: Underperforming. Metrics generally at/below benchmarks.
   - **"WE WASTED MONEY"**: Performing very poorly with significant spend. Pause candidate.
   - **"INSUFFICIENT DATA/SPEND"**: Spend < $50, not enough data.

**Input Data:**

**Ad Creative Performance Data (JSON):**
{{AD_DATA}}

**Overall Account Benchmark Data (JSON):**
{{BENCHMARK_DATA}}

**Output:** Return a JSON array where each element has:
- ad_id
- ad_name
- total_spend_creative
- performance_category (one of the 6 categories above)
- justification (2-3 sentences explaining the categorization with specific numbers)
- key_performance_indicators (object with the main metrics)
- recommendation (concise action: "Scale", "Pause", "Test", "Monitor", etc.)`,
...
store.clients.set(demoClientId, {
  ...
  facebookToken: 'demo-token',
  tokenExpiry: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString(),
  googleSheetId: '1_9fnWQm3ipnWg3DvP6XD-EnT2e5vZRvVlQyVA0rbNMQ',
  geminiApiKey: '',
  status: 'active',
...
const adNames = [
  'UGC Video - Happy Customer Testimonial',
  'Static Image - Product Hero Shot',
  'Carousel - Top 5 Products',
  'Video - Before & After Demo',
  'Static Image - Lifestyle Shot',
  'Video - Founder Story',
  'UGC - Unboxing Experience',
  'Static - Sale Promotion 30% OFF',
];

const demoLogId = 'log-demo-1';
const demoLog: ExecutionLog = {
  id: demoLogId,
  clientId: demoClientId,
  clientName: 'Demo E-commerce Store',
...
store.logs.set(demoLogId, demoLog);
...
adNames.forEach((name, i) => {
  ...
  const result: AdResult = {
    ...
    totalClicks: clicks,
    totalAddToCarts: Math.floor(purchases * 3.5),
    totalCheckoutsInitiated: Math.floor(purchases * 1.8),
    ...
    justification:
      cat === 'HELL YES'
        ? `With $${spend} spend driving ${purchases} purchases at a ROAS of ${(purchaseValue / spend).toFixed(2)}x, this ad dramatically outperforms the account benchmark.`
        : cat === 'WE WASTED MONEY'
        ? `Despite $${spend} in spend, this ad only generated ${purchases} purchases at a ROAS of ${(purchaseValue / spend).toFixed(2)}x, well below the account average.`
        : `This ad shows ${cat === 'YES' ? 'solid' : 'mixed'} performance with a ROAS of ${(purchaseValue / spend).toFixed(2)}x against the account benchmark.`,
    recommendation:
      cat === 'HELL YES' ? 'Aggressively Scale Budget & Explore New Audiences'
      : cat === 'YES' ? 'Scale Gradually -- Increase Budget 20%'
      : cat === 'MAYBE' ? 'Monitor Closely -- Test New Variations'
      : cat === 'WE WASTED MONEY' ? 'Pause Immediately'
      : cat === 'INSUFFICIENT DATA/SPEND' ? 'Wait for More Data'
